...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLED_Flag)
    {
        info->event[info->num] = EVENT_LED_Flag;
        info->num++;
        dataPoints->valueLED_Flag = gizStandardDecompressionValue(LED_Flag_BYTEOFFSET,LED_Flag_BITOFFSET,LED_Flag_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLED_Flag != cur->valueLED_Flag)
    {
        GIZWITS_LOG("valueLED_Flag Changed\n");
        ret = 1;
    }
    if(last->valueAlarm_Light != cur->valueAlarm_Light)
    {
        GIZWITS_LOG("valueAlarm_Light Changed\n");
        ret = 1;
    }
    ...
    if(last->valuePeople_flag != cur->valuePeople_flag)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuePeople_flag Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(LED_Flag_BYTEOFFSET,LED_Flag_BITOFFSET,LED_Flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED_Flag);
    gizStandardCompressValue(Alarm_Light_BYTEOFFSET,Alarm_Light_BITOFFSET,Alarm_Light_LEN,(uint8_t *)devStatusPtr,dataPoints->valueAlarm_Light);
    ...
    devStatusPtr->valueFire_flag = gizY2X(Fire_flag_RATIO,  Fire_flag_ADDITION, dataPoints->valueFire_flag); 
    devStatusPtr->valuePeople_flag = gizY2X(People_flag_RATIO,  People_flag_ADDITION, dataPoints->valuePeople_flag); 
